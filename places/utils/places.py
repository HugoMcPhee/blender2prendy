...
def setup_place_info_paths():
    ...
    place_info.place_folder_path = os.sep.join(place_info.parts[:-1])
    place_info.places_folder_path = os.sep.join(place_info.parts[:-2])
    place_info.assets_folder_path = os.sep.join(place_info.parts[:-3])
    place_info.renders_folder_path = place_info.place_folder_path + os.sep + "renders"
...
def setup_and_get_places_info():
    ...
    with os.scandir(place_info.places_folder_path) as directory_iterator:
        for directory in directory_iterator:
            if directory.is_dir():
                looped_place_has_main_file = False
                with os.scandir(
                    f"{place_info.places_folder_path}{os.sep}{directory.name}"
                ) as place_folder:
